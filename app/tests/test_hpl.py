...
def hpl_test_resources(db):
    ...
    problems = []
    activities = [
        Activity(url="http://example.com/3_Lawrence", name="3_Lawrence", type="html"),
        Activity(url="http://example.com/4_SNHU", name="4_SNHU", type="html"),
    ]
    ...
    kcs = [
        KnowledgeComponent(kc_id=kc_id, name=kc_id, mastery_prior=0.5)
        for kc_id in kc_ids
    ]
    for kc in kcs:
        kc.save()
    ...
    activities[0].knowledge_components.set(
        KnowledgeComponent.objects.filter(kc_id__in=kc_ids_1)
    )
    activities[1].knowledge_components.set(
        KnowledgeComponent.objects.filter(kc_id__in=kc_ids_2)
    )
...
def hpl_test_learner_lawrence(db):
    ...
    # case:
    learner = Learner(
        user_id="hpl_test_learner_lawrence", tool_consumer_instance_guid="default"
    )
    learner.save()
    Mastery.objects.bulk_create(
        [
            Mastery(
                learner=learner,
                knowledge_component=KnowledgeComponent.objects.get(
                    kc_id="sect_highered"
                ),
                value=0.9,
            ),
            Mastery(
                learner=learner,
                knowledge_component=KnowledgeComponent.objects.get(
                    kc_id="sect_mediatech"
                ),
                value=0.9,
            ),
            Mastery(
                learner=learner,
                knowledge_component=KnowledgeComponent.objects.get(kc_id="sect_prek12"),
                value=0.1,
            ),
            Mastery(
                learner=learner,
                knowledge_component=KnowledgeComponent.objects.get(kc_id="sect_policy"),
                value=0.1,
            ),
        ]
    )
...
def hpl_test_learner_snhu(db):
    ...
    # case:
    learner = Learner(
        user_id="hpl_test_learner_snhu", tool_consumer_instance_guid="default"
    )
    learner.save()
    Mastery.objects.bulk_create(
        [
...
def test_hpl_recommend_lawrence(
    engine_api, hpl_test_resources, hpl_test_learner_lawrence
):
    """
