    ...
    def validate_kc_id(self, value):
        ...
        if KnowledgeComponent.objects.filter(kc_id=value).exists():
            ...
            raise serializers.ValidationError(
                "Knowledge component with specified kc_id does not exist"
            )
...
class ScoreSerializer(serializers.ModelSerializer):
    ...
    activity = serializers.SlugRelatedField(
        slug_field="url", queryset=Activity.objects.all()
    )
    ...
    def create(self, validated_data):
        ...
        score = Score.objects.create(
            learner=learner, activity=activity, score=validated_data.pop("score")
        )
        return score
...
class ActivityRecommendationRequestSerializer(serializers.Serializer):
    ...
    collection = serializers.SlugRelatedField(
        slug_field="collection_id", queryset=Collection.objects.all()
    )
